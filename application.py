...
def plot_feature_compare(feat1: pd.DataFrame,
    ...
    fig.suptitle(f"Comparison of {feature} for Turbine 1 vs Turbine 2", fontsize=14)
...
with tab0:
    subtab = st.radio("Select View", ["Turbine 1", "Turbine 2","Time_Window_ Analysis"], key="subtab0")
    col = []
    axes = ["edge", "span", "flap"]
    blade = [1,2,3]
    for i in blade:
    ...
    if subtab == "Turbine 1":
        ...
        blades = [1, 2, 3]
        features = ['time_mean', 'time_rms', 'time_std',
       'time_skew', 'time_kurtosis', 'time_crest_factor',
       'time_impulse_factor', 'time_shape_factor', 'time_clearance_factor',
       'time_margin_factor', 'spec_dominant_freq', 'spec_dominant_amp',
       'spec_envelope_kurtosis']
        blade = st.selectbox("Select Blade", blades, index=0, key=f"blade_selector_tab0")
        feature = st.selectbox("Select Feature", features, index=0, key=f"feature_selector_tab0")
        ...
        for loc in ["root", "tip"]:
            for ax in axes:
                ...
                col.append(name)
        feat1 = extract_20s_features(df1[col], fs)
        feat2 = extract_20s_features(df2[col], fs)
        plot_feature_compare(feat1, feat2, feature=feature, channels=col, use_time_axis=False)
...
with tab2:
    ...
    # Welch PSD curves for D (two datasets overlaid per channel)
        ...
        selection = st.radio(
                                ...
                                options=list(mode_map.keys()),
                                horizontal=True,
                                format_func=lambda option: f"{option} ({mode_map[option]} Hz)", key='coh'
                            )
